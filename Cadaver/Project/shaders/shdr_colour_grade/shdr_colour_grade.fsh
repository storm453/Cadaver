...
uniform sampler2D u_lut_day;
uniform sampler2D u_lut_night;
uniform vec4 u_lutUvs_day;
uniform vec4 u_lutUvs_night;


uniform float u_time;

uniform float u_bf; // blend factor 0=day 1=night
...
vec4 lutRaw(sampler2D lut, vec4 lutUvs, vec2 uv) {
    return texture2D( lut, mix(lutUvs.xy, lutUvs.zw, uv) );
}

vec4 lutFiltered(sampler2D lut, vec4 lutUvs, vec2 uv)
{
    vec2 px = vec2(1)/lutRes;

    vec4 bl = lutRaw(lut, lutUvs, uv + vec2(0, 0) * px);
    vec4 br = lutRaw(lut, lutUvs, uv + vec2(1, 0) * px);
    vec4 tl = lutRaw(lut, lutUvs, uv + vec2(0, 1) * px);
    vec4 tr = lutRaw(lut, lutUvs, uv + vec2(1, 1) * px);
...
vec4 lutIndex(sampler2D lut, vec4 lutUvs, vec3 i) {
    ...
    return mix(
        lutFiltered(lut, lutUvs, uv0),
        lutFiltered(lut, lutUvs, uv1),
        imz);
}
...
void main()
{
    ...
    vec4 bfDaylu = lutIndex(u_lut_day, u_lutUvs_day, cab.rgb);    
    vec4 bfNiklu = lutIndex(u_lut_night, u_lutUvs_night, cab.rgb);

    gl_FragColor = mix(bfDaylu, bfNiklu, u_bf);
}